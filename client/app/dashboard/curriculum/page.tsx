      ...
      <Card className="border-red-900/50 bg-red-950/20 text-red-400 max-w-lg mx-auto mt-10">
        <CardHeader>
          ...
          <CardDescription className="text-red-300/80">
            Make sure the backend is active and your network connection is
            stable.
          </CardDescription>
        </CardHeader>
        <CardContent>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="border-slate-900 bg-slate-900/50">
          ...
          <CardContent>
            <div className="flex items-center gap-1.5 text-xs text-slate-400">
              <Calendar className="w-3.5 h-3.5 text-slate-500" /> Planned
              Schedule
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-slate-900 bg-slate-900/50">
          ...
          <CardContent>
            <div className="flex items-center gap-1.5 text-xs text-slate-400">
              <ShieldCheck className="w-3.5 h-3.5 text-green-500" /> SourceTrust
              Screened
            </div>
          </CardContent>
        </Card>
          ...
          {isGenerating ? (
            <Card className="border-slate-900 bg-slate-900/30 py-16 text-center">
              <CardContent className="space-y-4">
                ...
                <p className="text-sm text-slate-400 max-w-sm mx-auto">
                  Our Curriculum Architect and Scheduler agents are structuring
                  your syllabus modules. This screen will auto-refresh.
                </p>
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-6">
              {phases.map((phase: any, phaseIdx: number) => (
                <Card
                  key={phase.id}
                  className="border-slate-900 bg-slate-900/30"
                >
                  <CardHeader className="border-b border-slate-900/60 pb-3 bg-slate-900/10">
                    ...
                    <CardTitle className="text-base font-bold text-slate-200 mt-1">
                      {phase.title}
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="pt-4 space-y-4">
                    ...
                    {phase.modules?.map((mod: any) => (
                      <div
                        key={mod.id}
                        className="space-y-2 border-l-2 border-slate-800 pl-4 py-1"
                      >
                          ...
                          {mod.lessons?.map((les: any) => {
                            ...
                            return (
                              <div
                                key={les.id}
                                onClick={() =>
                                  handleLessonClick(les.id, les.status)
                                }
