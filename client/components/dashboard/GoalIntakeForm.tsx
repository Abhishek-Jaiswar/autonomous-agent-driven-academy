...
export function GoalIntakeForm({ onSubmit, isLoading }: GoalIntakeFormProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in py-4">
      ...
      <Card className="">
        <CardHeader>
          <CardTitle className="text-base">Initialize Study Academy</CardTitle>
        ...
        <CardContent className="space-y-5">
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {suggestions.map((s) => (
                  <div
                    ...
                    className="p-3 rounded-lg border border-border cursor-pointer transition-all duration-300"
                  >
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <Sparkles className="w-3.5 h-3.5 " />
                      <span className="text-xs font-bol">
                        {s.title}
                      </span>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
              <div className="space-y-2">
                <label className="text-xs font-semibold uppercase tracking-wider font-mono">
                ...
                <select
                  ...
                  disabled={isLoading}
                  className="w-full h-10 rounded-lg border border-border focus:outline-none"
                >
                  <option value="job_project">
                    Portfolio Project (Career)
                  </option>
                  <option value="exam_prep">Exam Preparation (Standard)</option>
                  <option value="school_subject">
                    Academic Subject (School)
                  </option>
                </select>
              </div>
